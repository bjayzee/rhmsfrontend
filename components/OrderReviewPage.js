...
const OrderReviewPage = () => {
  ...
  return (
    <div>
      ...
      <div className={`flex justify-between w-fit border border-[gray] p-4 m-2 rounded-xl ${cartItems?.length === 0 ? 'hidden' : ''}`}>
        {cartItems?.length > 0 && (
          <div>
      ...
      <div className='m-5'>
        ...
        <div className='flex justify-between'>
          ...
          <div className=' flex flex-row'>
            <TbCurrencyNaira className='w-5 h-6' />  <span>{totalPrice?.toLocaleString()}</span>
          </div>
        </div>
      </div>
      ...
      <div className="m-5 pl-10 font-bold">
        ...
        <PaystackButton className="paystack-button bg-rh-blue text-[white] px-8 py-2 rounded-full"
          {...componentProps}
